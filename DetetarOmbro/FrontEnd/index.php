...
source.onmessage = function(event) {
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
        ...
        const nomeInput = tr.querySelector('input[name="aluno_nome"]');
        if (idInput && nomeInput) {
            valoresInputs[idInput.value] = nomeInput.value;
        }
    });
    const data = JSON.parse(event.data);
    let novo = "";
    data.forEach(erro => {
        const valorAluno = valoresInputs[erro.id] || "";
        novo += `
            <tr class="table-warning">
                ...
                <td>
                    <form method="post" action="marcar_resolvido.php" class="d-flex align-items-center gap-2">
                        ...
                        <input type="text" name="aluno_nome" class="form-control form-control-sm" placeholder="Nome do aluno" required value="${valorAluno}">
